...
public class MediaPlaybackService extends Service {
    ...
    public static final String PLAYSTATE_CHANGED = "com.cvte.music.playstatechanged";
    public static final String META_CHANGED = "com.cvte.music.metachanged";
    public static final String QUEUE_CHANGED = "com.cvte.music.queuechanged";

    public static final String SERVICECMD = "com.cvte.music.musicservicecommand";
    ...
    public static final String TOGGLEPAUSE_ACTION = "com.cvte.music.musicservicecommand.togglepause";
    public static final String PAUSE_ACTION = "com.cvte.music.musicservicecommand.pause";
    public static final String PREVIOUS_ACTION = "com.cvte.music.musicservicecommand.previous";
    public static final String NEXT_ACTION = "com.cvte.music.musicservicecommand.next";
    public static final String CLOSE_ACITON = "com.cvte.music.musicservicecommand.close";
    public static final String ACTION_CLOSE_NOTIFICATION = "com.cvte.music.ACION_CLOSE_NOTIFICATION";

    private static final String ACTION_PLAYBACK_VIEWER = "com.cvte.music.action.PLAYBACK_VIEWER";

    public static final String WIDGET_CMD_TOGGLE_PAUSE = "com.cvte.music.widget_togglepause";
    public static final String WIDGET_CMD_NEXT = "com.cvte.music.widget_next";
    public static final String WIDGET_CMD_PREV = "com.cvte.music.widget_prev";

    public static final String ACTION_TIMER_STOP_CANCEL = "com.cvte.music.ACTION_TIMER_STOP_CANCEL";
    public static final String ACTION_TIMER_STOP_RESET = "com.cvte.music.ACTION_TIMER_STOP_RESET";
    ...
    private static final int TIMER_STOP = 21;
    ...
    private boolean mIsWidgetPlayClick;
    ...
    private static final int IDLE_DELAY = 0;// 60000;
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case TRACK_WENT_TO_NEXT:
                    ...
                    // add by zhangya to fix ArrayIndexOutOfBoundsException of -1
                    if (mPlayPos < 0) {
                        break;
                    }
                    // end of add
...
//                    updateNotification();
    ...
    private BroadcastReceiver mIntentReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (CMDNEXT.equals(cmd) || NEXT_ACTION.equals(action)) {
                ...
                if (mIsWidgetPlayClick) {
                    mIsWidgetPlayClick = false;
                } else {

                    int state = intent.getIntExtra("state", 1);
                    if (state == 0) { // headset plug out.
                        if (isPlaying()) {
                            pause(false);
                        }
                    }
    ...
    public void onCreate() {
        ...
        mCardId = MusicUtil.getCardId(this);
        ...
        commandFilter.addAction(ACTION_CLOSE_NOTIFICATION);
        registerReceiver(mIntentReceiver, commandFilter);

        IntentFilter timerStopFilter = new IntentFilter();
        timerStopFilter.addAction(ACTION_TIMER_STOP_CANCEL);
        timerStopFilter.addAction(ACTION_TIMER_STOP_RESET);
        registerReceiver(mTimerStopReceiver, timerStopFilter);
...
//                + Environment.getExternalStorageDirectory())));
    ...
    public void onDestroy() {
        ...
        unregisterReceiver(mTimerStopReceiver);
    ...
    private void reloadQueue() {
        ...
        if (qlen > 1) {
            ...
            Cursor crsr = MusicUtil.query(this,
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (intent != null) {
            ...
            MusicUtil.debugLog("onStartCommand " + action + " / " + cmd);

            if (CMDNEXT.equals(cmd) || NEXT_ACTION.equals(action)) {
                gotoNext(true);
            } else if (CMDPREVIOUS.equals(cmd)
                    || PREVIOUS_ACTION.equals(action)) {
                if (position() < 2000 || true) { // Roger: always prev
                    prev();
                } else {
                    seek(0);
                    play();
            ...
            } else if (CLOSE_ACITON.equals(action)) {
                if (isPlaying()) {
                    pause();
                    mPausedByTransientLossOfFocus = false;
                }
                sendBroadcast(new Intent(CLOSE_ACITON));
            } else if (CMDPAUSE.equals(cmd) || PAUSE_ACTION.equals(action)) {
                pause();
                mPausedByTransientLossOfFocus = false;
            } else if (CMDPLAY.equals(cmd)) {
                ...
                mPausedByTransientLossOfFocus = false;
                seek(0);
            } else if (WIDGET_CMD_PREV.equals(action)) {
                mIsWidgetPlayClick = true;
                prev();
            } else if (WIDGET_CMD_TOGGLE_PAUSE.equals(action)) {
                mIsWidgetPlayClick = true;
                if (isPlaying()) {
                    pause(false);
                    mPausedByTransientLossOfFocus = false;
            ...
            } else if (WIDGET_CMD_NEXT.equals(action)) {
                mIsWidgetPlayClick = true;
                gotoNext(true);
            }
        }
    ...
    private boolean openCurrentAndNext() {
        synchronized (this) {
            ...
            while (!open(MediaStore.Audio.Media.EXTERNAL_CONTENT_URI + "/"
                ...
                long id;
                try {
                    id = mCursor.getLong(IDCOLIDX);
                } catch (Exception e) {
                    Log.i("hanjiang", "open plist error");
                    e.printStackTrace();
                    return false;
                }
    ...
    private void setNextTrack() {
        ...
        if (mRepeatMode == REPEAT_CURRENT && mNextPlayPos >= 0) {
...
//            updateNotification();
    ...
    public void pause(boolean gotoIdle) {
        synchronized (this) {
            mMediaplayerHandler.removeMessages(FADEUP);
            if (isPlaying()) {
                mPlayer.pause();
    ...
    public void prev() {
        synchronized (this) {
            if (mShuffleMode == SHUFFLE_NORMAL) {
                // Roger add for change the prev button behavior
                if ("unknow".equals(System.getProperty("ro.music.shuffle.mode",
                        "unknow"))) {
                ...
                /*
                 * if (histsize > 1) { Integer pos = mHistory.get(histsize - 2);
                 * mHistory.remove(histsize - 1); mHistory.remove(histsize - 2);
                 * mPlayPos = pos.intValue(); } else { Integer pos =
                 * mHistory.remove(histsize - 1); mPlayPos = pos.intValue(); }
                 */
            } else {
                if (mPlayPos > 0) {
                    mPlayPos--;
    ...
    private int getNextPosition(boolean force) {
        if (mRepeatMode == REPEAT_CURRENT && isButtonNext == false) {
            if (mPlayPos < 0)
                return 0;
            return mPlayPos;
        }
    ...
    // cvt_zxl add
    public boolean isButtonNext = false;
    ...
    static class ServiceStub extends IMediaPlaybackService.Stub {
        ...
        // cvt_zxl add
        public void next2() {
            mService.get().isButtonNext = true;
            mService.get().gotoNext(true);
        }
        ...
        public String getDisplaytName() {
            return mService.get().getDisplaytName();
        }

        public String getDisplaytPath() {
            return mService.get().getDisplaytPath();
        }
    ...
    private BroadcastReceiver mTimerStopReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            String action = intent.getAction();
            if (ACTION_TIMER_STOP_CANCEL.equals(action)) {
                if (mMediaplayerHandler.hasMessages(TIMER_STOP)) {
                    mMediaplayerHandler.removeMessages(TIMER_STOP);
                }
            } else if (ACTION_TIMER_STOP_RESET.equals(action)) {
                if (mMediaplayerHandler.hasMessages(TIMER_STOP)) {
                    mMediaplayerHandler.removeMessages(TIMER_STOP);
                }
            }
